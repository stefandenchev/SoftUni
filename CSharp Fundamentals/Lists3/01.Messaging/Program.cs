...
namespace _01.Messaging
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //int result = [sum(int(digit) for digit in str(number)) for number in lst]
            ...
            for (int i = 0; i < numbers.Count; i++)
            {
                ...
                char currentChar = GetCharFromMessage(index, text);
                Console.Write(currentChar);

                int realIndex = CalculateRealIndex(index, text);
                string newMessage = text.Remove(realIndex, 1);
                text = newMessage;
            }
